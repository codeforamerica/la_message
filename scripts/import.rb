...
csv.each.with_index do |row, index|
  ...
  description = case row['DESCRIPTION']
                when "SSICompApp" then
                  "Proof of SSI"
                when "Earned Income-Interface" then
                  "Earned Income"
                when "Unsigned LAP Questionnaire" then
                  "Signed LaCHIP Affordable Plan Questionnaire"
                when "Social Security Number Unverified" then
                  "Proof of Social Security Number"
                when "CitizenshipInfo" then
                  "Proof of Citizenship"
                when "Unearned Income-Interface" then
                  "Unearned Income"
                when "US Citizenship" then
                  "Proof of Citizenship"
                when "Social Security Number missing" then
                  "Proof of Social Security Number"
                when "Bank Account" then
                  "Bank Statements"
                when "Life Insurance" then
                  "Proof of Life Insurance Policy"
                when "Burial Plot" then
                  "Burial Contract"
                when "Access to State Health Coverage" then
                  "Copies of Medical Insurance Cards"
                when "Incarceration Source" then
